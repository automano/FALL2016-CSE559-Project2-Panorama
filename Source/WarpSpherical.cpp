                                 ...
                                 float k1, float k2, const CTransform3x3 &r)
{
    ...
    for (int y = 0; y < dstSh.height; y++)
    {
        ...
        for (int x = 0; x < dstSh.width; x++, uv += 2)
        {
            ...
            float theta = (x - 0.5f*dstSh.width ) / f;
            float phi = (y - 0.5f*dstSh.height) / f;
			...
			//Image Stitching2 Page 23
			//convert to spherical coordinates
			xt = sin(theta)*cos(phi);
			yt = sin(phi);
			zt = cos(theta)*cos(phi);
			//project to z=1 plane
			p[0] = xt;
			p[1] = yt;
			p[2] = zt;
			p = r*p;//apply rotation matrix
			xt = p[0] / p[2];
			yt = p[1] / p[2];
			zt = 1;
			//apply radial distortion (05-camera-3 Page53)
